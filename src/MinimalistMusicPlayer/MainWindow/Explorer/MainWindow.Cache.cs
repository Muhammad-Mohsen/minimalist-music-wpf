	...
	public partial class MainWindow : Window
	{
		...
		public List<MediaItem> GetMediaItems(DirectoryInfo directory)
		{
			...
			if (MediaFileCache.ContainsKey(directory.FullName))
			{
				...
				if (directory.FullName != Player.PlaylistDirectory) cachedMediaItems.ForEach(i => { i.SetMediaIcon(false); i.SetTitleLabelForeground(false); });

				// else if at the playlist directory, select the currently playing item
				else MediaItem.Select(GetCachedMediaItemByPlaylistIndex(Player.Index, cachedMediaItems));
